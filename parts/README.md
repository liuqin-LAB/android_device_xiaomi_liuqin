...
6S Pro (liuqin). It follows the upstream LineageOS parts pattern: a
...
| Lift to check tablet | `pick_up_gesture_wakeup_mode` (System, 0/1) | Subscribes the stock SSC pickup sensor while the screen is off and adaptive sleep/screen attention is enabled, like the stock `PowerManagerServiceImpl`; the pickup event raises user activity and wakes with `WAKE_REASON_GESTURE` because LineageOS has no MIUI dim/AOD state |
...
- Charger/power-supply uevents (`PenUEventObserver`) detect the pen leaving
  ...
  during the stock state 4 pairing window is handled, stale bonds of other
  Smart Pen names are dropped like `MiuiBleOobHelperService.n2()`, and the
...
- Standard HID host connectivity and GATT battery, firmware and PnP reads
  ...
  firmware OTA.
- `StylusDeviceListener` is a port of the stock `MiuiStylusDeviceListener`:
...
transparent AIDL transport for `/dev/nanodev0`. `KeyboardController` ports
the stock `MiuiKeyboardManager` / `IICKeyboard` policy:
...
- the stock initialization chain: `0xA1` pogo status and MCU version on
  ...
  the local Bluetooth address) on the connection edge, and re-initialization
  on the `0x24` request / `0x20` recovery frames. Firmware upgrade and
  device authentication are deliberately omitted;
...
- the standard `InputManager` gate for the Nanosic input devices
  (`15d9:a1`..`15d9:a4`). The `0xa2` mouse interface is always disabled, as
  in stock `removeKeyboardDevicesIfNeeded`; the touchpad follows the
  keyboard enable state and receives the `0x21` feature command;
- the stock `iic_upgrade` partial wakelock keeps the 12 s `0x25` wake loop
  running for 120 s after connection and screen events, and the attach edge
...
- a transport error or binder death drops the proxy and reconnects every
  two seconds (`KeyboardNanoAppManager` keeps its binding for the whole
  boot on stock, so this only replaces its implicit trust in the client
  proxy).

The settings page shows connection, pogo voltage and firmware and exposes
the backlight brightness seekbar, auto-adjust and the stock
`enable_tap_touch_pad` tap-to-click switch.
