...
/**
 * Port of the stock `OnehopInfo` / `IICCommandMaker.getNfcData` /
 * `CommunicationUtil.sendNFC` chain used by `NFCTapFeature` on liuqin.
 *
 * The frames program the keyboard's NFC antenna with the tablet's OneHop
 * `MIRROR` payload so a phone tapping the antenna can start Xiaomi mirroring.
 * LineageOS has no OneHop/Mi Connect consumer, so the payload is sent for
 * protocol parity; the NFC touch report itself is handled by
 * `KeyboardProtocol.parseNfcTouched`.
 */
internal object NfcTapPayload {
    ...
    /** `OnehopInfo.toPayloadByteArray` TLV body. */
    ...
    /** `OnehopInfo.toProtocolByteArray` for the fixed `TAG_DISCOVERED` action. */
    ...
    /** `OnehopInfo.generateNFCProtocol`. */
    ...
    /** `IICCommandMaker.getNfcData` before the 52-byte chunk split. */
    private fun nfcData(nfc: ByteArray): ByteArray {
        ...
        all[4] = (((nfc.size + 1) shr 8) and 0x0ffe).toByte()
        all[7] = 84
        nfc.copyInto(all, 8)
        var sum = 84
        for (byte in nfc) {
