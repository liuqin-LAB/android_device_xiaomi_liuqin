...
class KeyHandler(private val context: Context) : DeviceKeyHandler {
    ...
    override fun handleKeyEvent(event: KeyEvent): KeyEvent? {
        if (event.keyCode == KeyEvent.KEYCODE_CAPS_LOCK) {
            if (event.action == KeyEvent.ACTION_UP &&
                event.device?.vendorId == VENDOR_NANOSIC
            ) {
                val capsLockOn = event.isCapsLockOn
                ...
                handler.post {
                    if (!rememberEvent(eventIdentity) || !powerManager.isInteractive) return@post
                    Settings.System.putIntForUser(
                        context.contentResolver,
                        CAPS_LOCK_STATE,
                        if (capsLockOn) 1 else 0,
    ...
    /**
     * The stock stylus shortcut is a two-step gesture: hold the configured
     * button, then touch the screen with the pen while the button remains
     * held. The original InputSettings mask window is MIUI-only; a read-only
     * gesture monitor reproduces its trigger semantics without stealing the
     * event from the foreground app.
     */
    private class StylusTouchMonitor(
        ...
        companion object {
            private const val MONITOR_NAME = "Liuqin stylus button monitor"
    ...
    companion object {
        ...
        private const val CAPS_LOCK_STATE = "liuqin_keyboard_caps_lock"
    }
}
