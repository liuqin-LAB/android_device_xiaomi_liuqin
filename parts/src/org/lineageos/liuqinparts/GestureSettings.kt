 ...
 *
 * Like the stock `TouchWakeUpFeatureManager`, double tap is OFF while
 * `gesture_wakeup_option` is unset, falling back to the legacy
 * `gesture_wakeup` switch (also off by default).
 *
    ...
    fun enabled(context: Context, key: String, user: Int = UserHandle.USER_CURRENT): Boolean {
        ...
        return when (key) {
            DOUBLE_TAP -> {
                ...
                if (option >= 0) option == DOUBLE_TAP_ON
                else Settings.System.getIntForUser(resolver, LEGACY_DOUBLE_TAP, 0, user) ==
