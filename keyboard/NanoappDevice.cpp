...
int NanoappDevice::writeCommand(const uint8_t* data, size_t size) {
    ...
    // The kernel returns the transport result, which is not always the byte
    // count (Hall and recovery requests complete with a short result). Match
    // the stock HAL: only a negative errno is a failure.
